...
# that are the minimum and maximum floors it can go to. Fill in the blanks to make the elevator go through 
# the floors requested.
...
# If you get a NameError message, be sure to run the Elevator class definition code block first. If you get an AttributeError message, 
# be sure to initialize self.current in your Elevator class.
...
# For the up and down methods, did you take into account the top and bottom floors? Keep in mind that the elevator shouldn't 
# go above the top floor or below the bottom floor. To check that out, try the code below and verify if it's working as expected. 
...
# Now add the str method to your Elevator class definition above so that when printing the elevator using the print( ) method, 
# we get the current floor together with a message. For example, in the 5th floor it should say "Current floor: 5"
